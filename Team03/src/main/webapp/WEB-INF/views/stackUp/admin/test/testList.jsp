...
<link rel="stylesheet" href="<c:url value="/resources/css/admin/test/testList.css" />">
...
<div id="tab-content">
	...
	<form id="deleteForm" action="<c:url value="/admin/test/deleteMultiple" />" method="post">
							...
							<c:forEach var="test" items="${testList}">
								<tr>
									<td><input type="checkbox" name="testNo"
										value="${test.testNo}" class="delete-checkbox" /></td>
									<td>${test.rnum}</td>
									<td class="text-start text-truncate" title="${test.testNm}">
										<a href="<c:url value="/admin/test/${test.testNo}" />" class="text-decoration-none text-dark">
											${test.testNm} 
										</a>
									</td>
									<td>
		...
		<div class="d-flex justify-content-between align-items-center mt-3">
			...
			<div class="d-flex gap-2">
				<a href="<c:url value="/admin/test/form" />" class="btn btn-primary">테스트 등록</a>
...
<form id="searchForm" action="<c:url value="/admin/test/list" />" method="get" style="display:none">
...
<script src='<c:url value="/resources/js/admin/test/testList.js" />'></script>