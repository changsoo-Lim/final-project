document.addEventListener("DOMContentLoaded", () => {
	...
	*/
	...
	// 요소 가져오기
	// 공고 선택 시 하위 모집 분야 업데이트
	jobSelect.addEventListener("change", () => {
		const selectedEmployNo = jobSelect.value;


		// 모집 분야 초기화
		fieldSelect.innerHTML = '<option value="">-- 분야를 선택하세요 --</option>';
		applicantCount.textContent = 0;

		// 선택된 공고 데이터 가져오기
		const selectedJob = empData.find(emp => emp.employNo === selectedEmployNo);

		if (selectedJob) {
			selectedJob.fieldList.forEach(field => {
				const option = document.createElement("option");
				option.value = field.filedNo;
				option.textContent = `${field.filedNm} (${field.filedRegion})`;
				fieldSelect.appendChild(option);
			});
		}
	});

	// 모집 분야 선택 시 지원자 수 업데이트
	fieldSelect.addEventListener("change", () => {
		const selectedEmployNo = jobSelect.value;
		const selectedFieldNo = fieldSelect.value;

		// 선택된 공고 및 모집 분야 데이터 가져오기
		const selectedJob = empData.find(emp => emp.employNo === selectedEmployNo);
		const selectedField = selectedJob?.fieldList.find(field => field.filedNo === selectedFieldNo);

		if (selectedField) {
			applicantCount.textContent = selectedField.applyCount;
		} else {
			applicantCount.textContent = 0;
		}
	});
