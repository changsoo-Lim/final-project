   ...
   // 문제 점수 변경 시 선택지의 itemScore 업데이트
	...
	$(document).on('click', '.file-upload-btn', function () {
	    ...
	    if (filePath && fileName) {
	        modalImage.attr('src', `${baseUrl}/images/compTest/${filePath}`); // 기존 파일 경로 사용
	...
	document.querySelectorAll("[data-atch-file-no][data-file-sn]").forEach(el => {
		el.addEventListener("click", async (e) => {
			...
			let url = `${baseUrl}/compTest/123/file/${atchFileNo}/${fileSn}`.replace(/\/+/g, '/');
			//console.log(url);
			try {
	...
	document.getElementById("deleteTest").addEventListener("click", async function (e) {
	    ...
	    if (deleteResult.isConfirmed) {
	        ...
	        const deleteUrl = `${baseUrl}/company/test/${testNo}`; // PathVariable URL 생성
	        ...
	        try {
	            ...
	            if (response.status === 200) {
	                ...
	                window.location.href = `${baseUrl}/company/test/list`; // 목록 페이지로 이동
	            }
	        } catch (error) {
